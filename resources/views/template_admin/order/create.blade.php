    ...
    <div class="col-12 col-md-12 col-lg-12">
        <div class="container">
            ...
            <div class="form-order">
                <div>
                    <label>Chọn cửa hàng để tạo order</label>
                    <select name="select_res" id="id_res" class="form-control select_res">
                        @foreach($get_res as $item)
                        <option value="{{ $item->id }}">{{ $item->name_res }} - {{ $item->address_res }} {{ $item->note_f }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <table class="table ">
                    <thead>
                        <tr class="table-danger" style="font-size: 15px">
                            <th colspan="3">Đơn hàng của quán</th>
                            <th colspan="4" id="name_res"></th>
                        </tr>
                    </thead>
                    <thead>
                    ...
                    <form action="{{ route('choose_food') }}" method="post" id="form_submit_order">
                        @csrf
                        <tbody>
...
<script type="text/javascript">
    ...
    $("#show_selected").click(function() {
        ...
        //  socket.on('server-say-hello', function(data) {
        console.log(socket);
        //  })
